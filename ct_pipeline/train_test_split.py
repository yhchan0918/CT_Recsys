...
from constants import REVIEWS_DATE_COL, FEATURE_COLS
...
def main_train_val_test(
    reviews, listings, reviewers, start_date, end_date, train_split_period_months
):
    ...
    train_listings, train_reviewers = build_partitioned_data(train_reviews, listings, reviewers)
    test_listings, test_reviewers = build_partitioned_data(test_reviews, listings, reviewers)
...
def build_partitioned_data(reviews, listings, reviewers):
    temp_listings = listings.copy()
    temp_reviewers = reviewers.copy()
    temp_reviews = reviews.copy()
    distinct_listings_in_reviews = temp_reviews["listing_id"].unique()
    distinct_reviewers_in_reviews = temp_reviews["reviewer_id"].unique()
    temp_reviewers = temp_reviewers[
        temp_reviewers["reviewer_id"].isin(distinct_reviewers_in_reviews)
    ]
    temp_listings = temp_listings[temp_listings["listing_id"].isin(distinct_listings_in_reviews)]

    return temp_listings, temp_reviewers
